...
1. **lmcode installable binary** — today lmcode runs from source via `bun run .../index.ts`. Package it as
...
2. **dogfood-driven hardening** — keep running real coding tasks through the fleet; fix whatever pain
...
3. **model-specific prompt + tool profiles** — let a provider/model select a specialized system prompt and
...
- durable-memory on-demand tool (`durable_memory`) — first-class tool to read, write, and append session durable memory (`durable-memory/index.md`) continuously during turns.
- Tier 1 in-process turn deadline (`executionGate.serialize`, `TurnTimeoutError` HTTP 504).
- Universal provider timeout defaults (header 300s, chunk idle 300s, total 1800s; terminal `HeaderTimeoutError`).
