...
- `console login [url]` (alias `account login`) `[--url url|-u url] [--output file|-o file] [--json]` — log in to console server via OAuth device authorization, with direct file export (`--output` / `-o`) and structured JSON output (`--json`).
- `console logout [email]` (alias `account logout`) `[--email email|-e email] [--force|-f] [--output file|-o file] [--json]` — log out from a console account with force guard, structured JSON confirmation, and direct file export (`--output` / `-o`).
- `console switch [org]` (alias `account switch`) `[--org org] [--output file|-o file] [--json]` — switch active organization across console accounts, with non-interactive org identifier support, structured JSON response, and direct file export (`--output` / `-o`).
...
- `debug rg files [--query q|-q q] [--glob g|-g g] [--limit n|-n n] [--output file|-o file] [--json]` / `debug rg search <pattern> [--glob g|-g g] [--limit n|-n n] [--output file|-o file] [--json]` — ripgrep fast file finding and pattern search across instance worktree, with direct file export (`--output` / `-o`) and structured JSON (`--json`).
...
- `debug agent <name> [--tool toolId|-t toolId] [--params json|-p json] [--output file|-o file] [--json]` — inspect agent configuration, permissions, and tool capabilities or execute tools directly with file export (`--output` / `-o`) and `--json`.
